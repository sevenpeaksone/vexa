...
Obsidian 스타일의 마크다운 에디터에 *Andrej Karpathy 의 LLM Wiki 패턴*을 통합했습니다.
...
[다운로드](#-다운로드) · [주요 기능](#-주요-기능) · [LLM Wiki](#-llm-wiki-워크플로우) · [단축키](#-단축키) · [소스 빌드](#-소스에서-빌드)
...
> 자세한 패턴은 [`docs/reference/llm-wiki.md`](docs/reference/llm-wiki.md) 와 [`sample-vault/_wiki/AGENTS.md`](sample-vault/_wiki/AGENTS.md) 참고.
...
```bash
# 요구: Node.js 18+, Rust 1.70+
npm install

# 개발 모드
npm run tauri:dev

# 프로덕션 빌드 (Windows NSIS)
npm run tauri:build
...
<div align="center">
<sub>Vexa — Obsidian 은 IDE, LLM 은 프로그래머, 위키는 코드베이스.</sub>
</div>
